...
const getPageContent = async (request: PageContentRequest) => {
  /**
...
export default async (
  ...
  if (req.method !== "POST") {
    throw new Error("Only accepts POSTs with body of { name: string }");
  }

  const pageContentRequest = req.body as PageContentRequest;

  if (typeof pageContentRequest?.page !== "string") {
